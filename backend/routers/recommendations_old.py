...
def recommend_jobs(seeker_id: int, db: Session = Depends(get_db)):
    recommended_jobs = get_top_recommendations(seeker_id, db, top_n=6)
...
#     recommendations = get_top_recommendations_from_data(seeker_data, db, top_n=6)
...
def recommend_jobs(seeker_data: RecommendationRequest, db: Session = Depends(get_db)):
    recommendations = get_top_recommendations_from_data(seeker_data.dict(), db, top_n=6)
    if not recommendations:
        raise HTTPException(status_code=404, detail="No recommendations found.")
    return recommendations